...
Neutrons are subatomic neutral particles emitted by radioactive materials. There are two primary sources of neutrons in the world. The overwhelming majority of neutrons come from cosmic sources, such as our own sun. The sun along with other stars can be thought of as an gigantic nuclear reactor that spew neutrons all over the universe. The other source of neutrons come from man-made sources such as reactors and nuclear weapons. It is presumed that with accurate modeling of cosmic neutrons, man-made sources can be detected. However, there are significant large spatio-temporal variations in cosmic neutrons impacting the Earth's surface. These variations also can potentially include weather and elevation effects. This report seeks to construct a statistical model that could be used in the future to create a map to model cosmic neutron penetration of the Earth. The consultant's collaboration partners hypothesize that Earth's surface neutron flux is primarily dependent (in order of importance) on elevation above sea level, longitudinal/latitudinal variation due to the Earth's magnetic field, and weather effects such as moisture content.
...
\section{Results}
Moran's I is used to test for the presence of spatial auto-correlation. Looking at Figure 3 while setting a significance level of .05 (the red line), it shows that a very small minority (25 total) of the days showed evidence of spatial auto-correlation. 
...
\noindent
We can see that Altitude, Precipitation Intensity and Visibility are significant in Table 1. The measure of goodness of fit, $R^{2}$ is \Sexpr{summary(test)$r.squared} which indicates poor overall predictive capability. Perhaps this is due to the zero counts as seen in Figure 2. Removing these zero count values yields an improved model where $R^{2}$ rises to \Sexpr{summary(test2)$r.squared}, Pressure and Humidity also become significant as seen in Table 2.
...
\noindent
Finally, stripping the observations where there is missing weather data as \href{http://forecast.io}{Forecast.io} does not provide complete data in every situation, yields non-spatial Model 3. The $R^{2}$ becomes \Sexpr{summary(test3)$r.squared} meaning the model can explain approximately \Sexpr{round(summary(test3)$r.squared * 100,0)}\% of the neutron counts. Table 3 shows the coefficients along with standard errors. Interpretation is straightforward. Holding all other covariates constant, a one meter increase in altitude will yield 7.88 more neutrons per hour for that day.  
